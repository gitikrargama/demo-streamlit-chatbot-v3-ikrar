...
st.title("🥗 FitBot")
st.caption("Asisten Diet & Gaya Hidup Sehat Kamu 😊")
...
if not google_api_key:
    st.error("Google API Key tidak ditemukan. Pastikan sudah diatur di Streamlit Secrets atau file .env.")
    st.stop()
...
if not st.session_state.messages:
    with st.chat_message("assistant"):
        st.markdown("Halo! Aku **FitBot**, siap bantu kamu hidup lebih sehat 🥗✨\n\nCeritain dulu deh, kamu lagi fokus apa nih — nurunin berat, jaga pola makan, atau sekadar hidup lebih sehat?")
...
prompt = st.chat_input("🍎 Ceritakan apa yang kamu makan hari ini atau tanyakan seputar diet...")
...
if prompt:
    ...
    try:
        # Tambahkan konteks agar AI tahu perannya sebagai asisten diet pribadi
        full_prompt = f"""
        Kamu adalah FitBot, asisten gaya hidup dan diet pribadi.
        Tugasmu:
        - Jawab dengan ramah, singkat, dan memberi motivasi positif.
        - Jika pengguna menyebut makanan, perkirakan kalorinya (secara umum).
        - Jika pengguna menyebut berat badan atau olahraga, berikan dukungan dan tips ringan.
        - Jangan terlalu kaku seperti dokter; gunakan gaya santai tapi informatif.
